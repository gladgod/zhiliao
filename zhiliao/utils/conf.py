...
from .timezone import get_best_local_timezone
...
def set_dynamic_settings(s):
    ...
    add_to_builtins("zhiliao.template.loader_tags")
    ...
    # cp = "zhiliao.pages.context_processors.page"
    # if ("zhiliao.pages" in s["INSTALLED_APPS"] and
    ...
    # zhiliao.accounts is being used.
    if "zhiliao.accounts" in s["INSTALLED_APPS"]:
        auth_backend = "zhiliao.core.auth_backends.MezzanineBackend"
    ...
    if "zhiliao.blog" in s["INSTALLED_APPS"]:
        append("INSTALLED_APPS", "zhiliao.generic")
    if "zhiliao.generic" in s["INSTALLED_APPS"]:
        s.setdefault("COMMENTS_APP", "zhiliao.generic")
        append("INSTALLED_APPS", "django_comments")

    # Ensure mezzanine.boot is first.
    try:
        move("INSTALLED_APPS", "zhiliao.boot", 0)
    except ValueError:
        pass
