...
from zhiliao.conf import registry
from .importing import import_dotted_path, path_for_import
...
def build_deploy_docs(docs_path):
    try:
        ...
        return
    project_template_path = path_for_import("zhiliao.project_template")
...
def build_changelog(docs_path, package_name="zhiliao"):
    """
...
def build_modelgraph(docs_path, package_name="zhiliao"):
    """
    ...
    apps = [a.rsplit(".")[1] for a in settings.INSTALLED_APPS
            if a.startswith("zhiliao.") or a.startswith(package_name + ".")]
    try:
...
def build_requirements(docs_path, package_name="zhiliao"):
    """
