...
from zhiliao.core.sitemaps import DisplayableSitemap
...
urlpatterns += patterns("",
    ("^", include("zhiliao.core.urls")),
    ("^", include("zhiliao.generic.urls")),
)
...
if _old_accounts_enabled:
    ...
    warnings.warn("The setting ACCOUNTS_ENABLED is deprecated. Please "
                  "add zhiliao.accounts to INSTALLED_APPS.")
if _old_accounts_enabled or "zhiliao.accounts" in settings.INSTALLED_APPS:
    ...
    urlpatterns += patterns("",
        ("^", include("zhiliao.accounts.urls")),
    )

# Mezzanine's Blog app.
blog_installed = "zhiliao.blog" in settings.INSTALLED_APPS
if blog_installed:
    BLOG_SLUG = settings.BLOG_SLUG.rstrip("/")
    blog_patterns = patterns("",
        ("^%s" % BLOG_SLUG, include("zhiliao.blog.urls")),
    )
    urlpatterns += blog_patterns

# Mezzanine's Pages app.
PAGES_SLUG = ""
if "zhiliao.pages" in settings.INSTALLED_APPS:
    ...
    if blog_installed and not BLOG_SLUG:
        ...
        urlpatterns[blog_patterns_start:len(blog_patterns)] = patterns("",
            ("^%s" % str(PAGES_SLUG), include("zhiliao.pages.urls")),
        )
    else:
        urlpatterns += patterns("",
            ("^", include("zhiliao.pages.urls")),
        )
