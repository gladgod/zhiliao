...
from .functions import (get_path, get_breadcrumbs,
    get_filterdate, get_settings_var, get_directory, convert_filename)
from .templatetags.fb_tags import query_helper
from .base import FileObject
from .decorators import flash_login_required
...
mixin_class_name = "filebrowsersafe.storage.%sMixin" % storage_class_name

# Workaround for django-s3-folder-storage
if django_settings.DEFAULT_FILE_STORAGE == 's3_folder_storage.s3.DefaultStorage':
    mixin_class_name = 'filebrowsersafe.storage.S3BotoStorageMixin'

try:
    # mixin_class = import_dotted_path(mixin_class_name)

    module = import_module('zhiliao.filebrowsersafe.storage')
    member_name = 'FileSystemStorageMixin'
    mixin_class = getattr(module, member_name)
